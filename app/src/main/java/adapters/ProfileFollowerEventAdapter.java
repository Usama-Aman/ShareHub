...
/**
 * Adapter to set event list followed by a user
 */


public class ProfileFollowerEventAdapter extends RecyclerView.Adapter<ProfileFollowerEventAdapter.EventsVH> {

    private ArrayList<String> eventImageList = new ArrayList<String>();
    private ImageSliderAdapter imageSliderAdapter;

    public interface OnItemClickListener {
        void onCancelEvent(final int position);

        void onAttendEvent(final int position);

    }

    private ArrayList<UserFollowersEventResponse> events;
    private Context context;
    private String mMonth, mWeek;
    Date date;
    OnItemClickListener listener;

    public ProfileFollowerEventAdapter(Context context, ArrayList<UserFollowersEventResponse> eventListData, OnItemClickListener listener) {
        this.context = context;
        events = eventListData;
        this.listener = listener;
    }

    public List<UserFollowersEventResponse> getEvents() {
        return events;
    }

    public void setEvents(ArrayList<UserFollowersEventResponse> events) {
        this.events = events;
    }
    ...
    @Override
    public void onBindViewHolder(EventsVH holder, final int position) {

        final UserFollowersEventResponse event = events.get(position);
...
//set views according to eventType
        ...
        if (!eventType.equalsIgnoreCase("past")) {
            if (event.getIsAttending() == 0 || event.getIsAttending() == 2) {
        ...
        eventsVH.mEventAttendingLiveStatus.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (SharedPreference.isUserLoggedIn(context)) {
                    eventsVH.mEventAttendingLiveStatus.setVisibility(View.GONE);
                    eventsVH.mEventCancelLiveStatus.setVisibility(View.VISIBLE);
                    listener.onAttendEvent(position);
                } else {
        ...
        eventsVH.mEventCancelLiveStatus.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (SharedPreference.isUserLoggedIn(context)) {
                    ...
                    ProfileFollowerEventAdapter.this.notifyDataSetChanged();
                    listener.onCancelEvent(position);
                } else {
                    Intent act = new Intent(context, MainActivity.class);
                    context.startActivity(act);
                }
